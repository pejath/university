...
DATE = '1999.12.12'
SUBJECTS = %w[Math Russian Geography English Economy IT PE Physics Biology Chemistry Science].freeze
FACULTS.each do |facult|
  Faculty.create(name: facult,
                 formation_date: "19#{rand(70..99)}.#{month = rand(1..12)}.#{month != 2 ? rand(1..28) : rand(1..30)}")
end
...
30.times do
  Group.create(department_id: rand(1..Department.count), specialization_code: rand(400), course: rand(1..5),
               form_of_education: FORM_OF_EDUCATION.sample, curator_id: rand(1..Lecturer.count))
end
...
50.times do
  ...
  lec = Lecture.new(auditorium: rand(900), corpus: rand(1..4), lecture_time_id: rand(1..8), group_id: rand(1..15),
                    lecturer_id: lecturer, subject_id: subject, weekday: DAY.sample)
  lec.save if lec.valid?
end

(1..Student.count).each do |i|
  5.times do
    Mark.create(subject_id: rand(1..Subject.count), mark: rand(1..5), student_id: i,
